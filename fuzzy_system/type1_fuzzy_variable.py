...
class Type1FuzzyVariable():
    ...
    def _adjust_value(self, value):
        ...
        if value < self._min_val:
            ...
            value = round((round((value - self._min_val) / self._delta) * self._delta) + self._min_val,
                          fuzzy_system.system_settings.PRECISION)
    ...
    def generate_sets_mean(self, n, mean):
        ...
        s = Type1FuzzySet.create_trapezoidal(self._min_val, self._max_val, self._res,
                                             self._max_val - set_half_support_post, self._max_val, self._max_val,
                                             set_name)
        self._add_set(set_name, s)
    ...
    def get_dom(self, variable_value, f_set_name):

        variable_value = self._adjust_value(float(variable_value))

        res_dom = -1

        for name, f_set in self._sets.items():
            if name == f_set_name:
                dom = f_set[variable_value]
                if dom > res_dom:
                    res_dom = dom
